//! Walrus channel adapters.
//!
//! Defines the [`Channel`] trait, platform types, routing, and concrete adapters
//! (currently Telegram).
...
pub trait Channel: Send + Sync {
    ...
    fn connect(
        &self,
        config: Self::Config,
    ) -> impl Future<Output = Result<impl Stream<Item = Self::Event> + Send>> + Send;

    /// Send a message through the channel.
    fn send(&self, message: ChannelMessage) -> impl Future<Output = Result<()>> + Send;
}
