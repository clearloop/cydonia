...
//! concurrent execution. Tools are stored in a shared [`ToolRegistry`]
//! behind `Arc<RwLock>` supporting post-startup registration (e.g. MCP
//! hot-reload).

use crate::{
    Agent, AgentBuilder, AgentConfig, AgentEvent, AgentResponse, AgentStopReason,
    agent::tool::{Handler, ToolRegistry},
    model::{Message, Model, Tool},
    runtime::hook::Hook,
};
use anyhow::Result;
...
impl<M: Model + Send + Sync + Clone + 'static, H: Hook + 'static> Runtime<M, H> {
    ...
    pub fn add_agent(&mut self, config: AgentConfig) {
        ...
        let agent = AgentBuilder::new(self.model.clone()).config(config).build();
